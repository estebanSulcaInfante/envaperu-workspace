...
*   **Ingeniero de Procesos:** Define y mantiene las recetas estándar de cada color-familia (proporciones de materia prima, gramajes de colorante por bolsa de 25kg). Es quien valida que la composición sea correcta.
...
*   **Resolución (confirmada):** Los aditivos se dosifican en **gramos por bolsa de 25kg**, igual que los colorantes. Por lo tanto, se modelan dentro de la misma tabla `Colorante` añadiendo un campo `tipo` con valores: `'COLORANTE'` (default) o `'ADITIVO'`. Esto evita crear una tabla nueva y permite reutilizar la misma lógica de dosificación.
...
*   **Resolución:** En la receta normalizada, los colorantes se registran en **gramos por bolsa de 25kg** (que es la unidad estándar de la industria plástica). Al crear un lote real, el sistema escala proporcionalmente según los kg reales del lote. Las materias primas siguen usando fracciones (que son independientes del peso total).
...
*   **And** define los colorantes: Azul Ultramar EP-24 = 120g, Dióxido de Titanio 2220 = 10g (por bolsa de 25kg)
